...
$BoutonExecute.Location = [System.Drawing.Point]::new(($form.Width - $BoutonExecute.Width) / 2 - 115, $searchBox.Bottom + 5)
...
$BoutonExecute.Add_Click({
    if (-not $menu.SelectedItem) {
        ...
        $cheminFichier = "A:\PS_ANA-Logs\config\Variables\Script_a_Lancer\$($env:COMPUTERNAME)_$($env:USERNAME).csv"
        ...
        $nouvelleLigne = "A:\PS_ANA-Logs\Script\$selectedItem"
        ...
        if ($confirmation -eq [System.Windows.Forms.DialogResult]::Yes) {
            ...
            Start-Process powershell.exe -ArgumentList "-NoExit", "-ExecutionPolicy Bypass", "-File A:\PS_ANA-Logs\config\LogsScriptDeCaptureFenetrePowershell.ps1"   -Verb RunAs
        } elseif ($confirmation -eq [System.Windows.Forms.DialogResult]::No) {
            # Appeler le script normalement
            Start-Process powershell.exe -ArgumentList "-NoExit", "-ExecutionPolicy Bypass", "-File A:\PS_ANA-Logs\config\LogsScriptDeCaptureFenetrePowershell.ps1", "-selectedScript $($menu.SelectedItem)" 
        } else {
