...
if (Test-Path $NomFichierLanceur) {
    ...
    if ($Reponse.ToLower() -eq "o") {
        ...
        write-host "voulez-vous remettre à zéro l'installation (supprime tous les Logs) O/N ?"
        $Reponse = Read-Host "O/N"
        if ($Reponse.ToLower() -eq "o") {
            . ".\config\ParametrageApplication\RemiseAzero.ps1"
        }
    } else {
...
. .\config\ParametrageApplication\NomApplication.ps1
...
. ".\config\ParametrageApplication\CreationIcone.ps1" 
...
. ".\config\ParametrageApplication\CreationRaccourci.ps1"

##########################################
#    changement du logo dans l'appli     #
##########################################

. ".\config\ParametrageApplication\ChangementLogoDansLapplication.ps1"
...
. ".\config\ParametrageApplication\ModificationDesCouleurs.ps1"

##########################################
#        Création de _lanceur.bat        #
##########################################

. ".\config\ParametrageApplication\CreationDuLanceur.ps1"
